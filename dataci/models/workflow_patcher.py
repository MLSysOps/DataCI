...
ENTRY_FUNC_REPLACE_TEXT = 'Generated by dataci: replace entry function'
...
def replace_entry_func(basedir: Path, source: 'Stage', target: 'Stage'):
    new_mountdir = basedir / target.name
    source_func_node = source.script.entry_node
    ...
    padding = entryfile_script.splitlines()[0].encode()[:source_func_node.col_offset].decode()
    print(f"Modify file '{modify_file}'")
    new_file_script = modify_file_script.replace(
        entryfile_script, f'{padding}# {ENTRY_FUNC_REPLACE_TEXT}'
    )
    for text in difflib.unified_diff(modify_file_script.splitlines(), new_file_script.splitlines()):
        if text[:3] not in ('---', '+++', '@@ '):
            print(text)
    ...
    for path in paths:
        ...
        for node, global_import_name, var_name in import_points:
            ...
            for line in script.splitlines():
                if ENTRY_FUNC_REPLACE_TEXT in line:
                    remove_line = line
                    break
            else:

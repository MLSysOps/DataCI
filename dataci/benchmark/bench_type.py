...
def verify_bench_type(bench_type: str, train_dataset: 'Dataset', test_dataset: 'Dataset'):
    ...
    if bench_type == 'data_augmentation':
        exceptions = list()
        if not text_augmentation_checker(train_dataset):
            exceptions.append(f'train_dataset {train_dataset} is not generated by text augmentation')
        if not (text_augmentation_checker(test_dataset) or standalone_checker(test_dataset)):
            exceptions.append(f'test_dataset {test_dataset} is not generated by text augmentation or standalone')
        if not same_train_val_checker(train_dataset, test_dataset):
            exceptions.append(
                f'train_dataset {train_dataset} and test_dataset {test_dataset} are not two '
                f'splits of a common parent dataset'
            )
    elif bench_type == 'data_selection':
        exceptions = list()
        if not text_selection_checker(train_dataset):
            exceptions.append(f'train_dataset {train_dataset} is not generated by text selection')
        if not (text_selection_checker(test_dataset) or standalone_checker(test_dataset)):
            exceptions.append(f'test_dataset {test_dataset} is not generated by text selection or standalone')
        if not same_train_val_checker(train_dataset, test_dataset):
            exceptions.append(
                f'train_dataset {train_dataset} and test_dataset {test_dataset} are not two '
                f'splits of a common parent dataset'
        ...
        exceptions = [f'Unsupported benchmark type: {bench_type}']

    if len(exceptions) > 0:
        raise ValueError(
            f'Invalid benchmark type {bench_type} for train_dataset {train_dataset} and test_dataset {test_dataset}:\n'
            "\n".join([f'- {exception}' for exception in exceptions])
        )
    return True
